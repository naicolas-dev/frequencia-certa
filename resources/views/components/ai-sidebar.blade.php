<div x-data="aiSidebar" @open-ai-sidebar.window="openSidebar($event.detail)"
    ...
    <div class="fixed inset-y-0 right-0 w-full md:max-w-md bg-white dark:bg-[#0B1220] shadow-2xl transform transition-transform duration-300 ease-in-out flex flex-col"
        ...
        <div class="p-4 border-t border-gray-100 dark:border-gray-800 bg-white dark:bg-[#0B1220] flex-shrink-0">
            <template x-if="!showSubjects">
                <div class="grid grid-cols-2 gap-3">
                    <button @click="consultarIaDia()"
                        ...
                        <div class="w-8 h-8 rounded-full bg-white dark:bg-gray-800 flex items-center justify-center shadow-sm mb-2 group-hover:scale-110 transition">
                            📅
                        </div>
                        <span class="text-xs font-bold">Posso faltar hoje?</span>
                    </button>
                    
                    <button @click="openSubjectSelector()"
                            :disabled="typing"
            ...
            <template x-if="showSubjects">
                <div class="flex flex-col h-full animate-fade-in-up">
                    <div class="flex justify-between items-center mb-3">
                        ...
                        <button @click="showSubjects = false" class="text-xs text-gray-400 hover:text-gray-600 dark:hover:text-gray-200">Cancelar</button>
                    </div>
                    ...
                    <div class="max-h-40 overflow-y-auto space-y-2 pr-1 custom-scrollbar">
                         ...
                         <template x-for="sub in subjects" :key="sub.id">
                            <button @click="consultarIaMateria(sub)" 
                                ...
                                <span class="text-sm font-medium text-gray-700 dark:text-gray-200" x-text="sub.nome"></span>
            ...
            <p x-show="!showSubjects" class="text-[10px] text-center text-gray-400 mt-3 font-medium">O Oráculo utiliza seus créditos de sabedoria.</p>
        </div>
    </div>
</div>
                ...
                // 1. Confirmação
                 if (window.swalTailwind) {
                    const result = await window.swalTailwind.fire({
                        ...
                        html: `O Oráculo analisará todas as aulas de HOJE (${dataFormatada}).<br><span class="text-sm text-purple-600 dark:text-purple-400 font-bold">Custo: 5 créditos</span>`,
                        icon: 'question',
            ...
            async consultarIaMateria(subject) {
                ...
                // 1. Confirmação
                 if (window.swalTailwind) {
                    const result = await window.swalTailwind.fire({
                        title: 'Invocar o Oráculo?',
                        html: `O Oráculo analisará <strong>${subject.nome}</strong>.<br><span class="text-sm text-purple-600 dark:text-purple-400 font-bold">Custo: 10 créditos</span>`,
                        icon: 'question',
                        showCancelButton: true,
                        confirmButtonText: 'Sim, invocar!',
                        cancelButtonText: 'Cancelar',
