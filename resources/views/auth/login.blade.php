...
<body
  ...
  <div class="fixed top-0 left-0 w-full h-full overflow-hidden -z-10 pointer-events-none">
    <div
      class="absolute top-0 left-1/4 w-96 h-96 bg-blue-400/20 rounded-full mix-blend-multiply filter blur-3xl opacity-50 animate-blob dark:bg-blue-900/20">
    </div>
    <div
      class="absolute top-0 right-1/4 w-96 h-96 bg-purple-400/20 rounded-full mix-blend-multiply filter blur-3xl opacity-50 animate-blob animation-delay-2000 dark:bg-purple-900/20">
    </div>
    <div
      class="absolute -bottom-32 left-1/3 w-96 h-96 bg-emerald-400/20 rounded-full mix-blend-multiply filter blur-3xl opacity-50 animate-blob animation-delay-4000 dark:bg-emerald-900/20">
    </div>
  </div>

  <div x-cloak x-show="toast.open" x-transition class="fixed top-6 left-1/2 -translate-x-1/2 z-[60] w-[min(92vw,520px)]"
    role="status" aria-live="polite">
    <div class="rounded-2xl border shadow-lg backdrop-blur-md px-4 py-3"
  ...
  <main class="relative z-10 w-full max-w-[400px] px-4 py-10">
    <div
      class="bg-white/70 dark:bg-gray-900/70 backdrop-blur-xl rounded-[2.5rem] shadow-2xl border border-white/20 dark:border-gray-800 p-8 sm:p-10 relative overflow-hidden transition-all duration-500">
      <div class="text-center mb-6">
        <div
          class="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-gradient-to-tr from-blue-600 to-blue-500 text-white shadow-lg shadow-blue-500/30 mb-5 transform -rotate-3 hover:rotate-0 transition-transform duration-300">
      ...
      @if (session('status'))
        <div
          class="mb-6 flex items-center gap-3 p-4 rounded-xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800 text-emerald-700 dark:text-emerald-400 animate-fade-in-down"
          role="status" aria-live="polite">
      ...
      <div class="space-y-3 mb-6">
        <button type="button" @click="handleSocial('google')" :disabled="loading"
        ...
        <button type="button" @click="handleSocial('github')" :disabled="loading"
          :class="{ 'opacity-50 pointer-events-none cursor-not-allowed': loading }"
      ...
      <form method="POST" action="{{ route('login') }}" class="space-y-5" @submit="loading = true">
        ...
        <div>
          ...
          <x-input-error :messages="$errors->get('email')"
            class="mt-2 text-sm font-medium text-red-600 dark:text-red-400 ml-1 animate-pulse" />
        </div>
        ...
        <div>
          ...
          <div class="relative">
            ...
            <button type="button" @click="showPassword = !showPassword"
              ...
              :aria-pressed="showPassword.toString()" aria-label="Mostrar/ocultar senha">
              ...
              <svg x-cloak x-show="showPassword" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                  d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.542-7a10.059 10.059 0 011.517-2.925m2.766-2.541A9.996 9.996 0 0112 5c4.478 0 8.268 2.943 9.542 7a10.057 10.057 0 01-1.127 2.18m-4.545 3.328A3 3 0 0112 15a3 3 0 01-3-3 3 3 0 01.373-1.46m-1.795-3.076A3.385 3.385 0 0112 9a3.385 3.385 0 012.352 1.056m-5.322-2.31L17.5 17.5">
                </path>
              </svg>
            </button>
          </div>
        ...
        <div class="flex items-center justify-between mt-4">
          <label for="remember_me" class="flex items-center cursor-pointer group select-none py-2 -my-2 pr-2">
            <input id="remember_me" type="checkbox"
              class="w-5 h-5 rounded border-gray-300 dark:border-gray-600 text-blue-600 shadow-sm focus:ring-blue-500 cursor-pointer transition-transform checked:scale-110"
              name="remember" />
            <span
              class="ms-2 text-sm font-medium text-gray-600 dark:text-gray-300 group-hover:text-gray-900 dark:group-hover:text-white transition-colors">Lembrar
              de mim</span>
          </label>

          @if (Route::has('password.request'))
            <a class="text-sm font-bold text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 transition underline underline-offset-4 decoration-blue-300/50 p-2 -mr-2"
  ...
  <script>
    function loginPage() {
      ...
      return {
        ...
        // usado pra destravar rápido quando o popup fecha (sem esperar Firebase rejeitar)
        _attempt: 0,
        ...
        async handleSocial(provider) {
          ...
          const unlock = () => {
            // destrava somente se ainda for a tentativa atual
            if (this._attempt === attemptId) this.loading = false;
          };

          // Remover o listener de focus que destrava prematuramente
          ...
          try {
            ...
            // cancelado / throttle / etc.
            if (res?.ignored) return;

            // redirecionou (popup-blocked -> redirect)
            if (res?.redirected) return;
          } catch (e) {
            if (!silentCodes.has(e?.code)) {
              this.showToast(e?.message || "Erro ao entrar. Tente novamente.", "error");
            }
          } finally {
